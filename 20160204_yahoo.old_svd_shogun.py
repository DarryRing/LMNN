...
candidate_n_train_lmnn = [5000,6000,7000,8000,9000,10000]
candidate_n_neighbors = [5]
...
for i in range(len(labels)):
    dic_index_label[i] = labels[i]
    dic_label_index[labels[i]] = i
all_train_labels_index = numpy.zeros(len(all_train_labels))
for i in range(len(all_train_labels)):
    all_train_labels_index[i]=dic_label_index[all_train_labels[i]]#string to index
all_test_labels_index = numpy.zeros(len(all_test_labels))
for i in range(len(all_test_labels)):
    all_test_labels_index[i]=dic_label_index[all_test_labels[i]]#string to index
del i
...
def classify_and_test(filename, n_neighbors, train_vectors, train_labels, test_vectors, test_labels):
    ...
    dic_day_accuracy = {}
    dic_day_estimated_labels = {}
    dic_day_test_labels_string = {}
    dic_day_estimated_labels_string = {}
    for day in sorted(dic_day_test_labels.keys()):
        length = len(dic_day_test_labels[day])
        dic_day_accuracy[day] = classifier.score(dic_day_test_vectors[day], dic_day_test_labels[day])
        dic_day_estimated_labels[day] = classifier.predict(dic_day_test_vectors[day])
        indicators = numpy.ones(length, numpy.int8)
        dic_day_test_labels_string[day] = numpy.empty(length, dtype=object)
        dic_day_estimated_labels_string[day] = numpy.empty(length, dtype=object)
        for i in range(length):
            if dic_day_estimated_labels[day][i] != dic_day_test_labels[day][i]: indicators[i] = 0#outputer of right or wrong
            dic_day_test_labels_string[day][i] = dic_index_label[dic_day_test_labels[day][i]]#index to string
            dic_day_estimated_labels_string[day][i] = dic_index_label[dic_day_estimated_labels[day][i]]#index to string
            del i
        path_name = path_daytype + 'labels/'
        if not os.path.exists(path_name): os.mkdir(path_name)
        result = pandas.DataFrame(numpy.array([dic_day_test_labels_string[day], dic_day_estimated_labels_string[day], indicators]).T)
    ...
    del day
    return(dic_day_test_labels_string, dic_day_estimated_labels_string, dic_day_accuracy)
...
def run_without_lmnn(label, n_neighbors, train_vectors, train_labels, test_vectors, test_labels):
    ...
    (dic_day_test_labels, dic_day_estimated_labels, dic_day_accuracy) = classify_and_test(filename, n_neighbors, train_vectors, train_labels, test_vectors, test_labels)
    return (filename, dic_day_test_labels, dic_day_estimated_labels, dic_day_accuracy)
...
if strategy == 'together':
    ...
    if is_run_without_lmnn:
        for n_neighbors in candidate_n_neighbors:
            t = time.time()
            (filename, dic_day_test_labels, dic_day_estimated_label, dic_day_accuracy) = run_without_lmnn('together', n_neighbors, train_vectors, all_train_labels_index, test_vectors, all_test_labels_index)
    ...
    for n_train_lmnn in candidate_n_train_lmnn:
        ...
        train_lmnn_labels = all_train_labels_index[::len(all_train_labels)/n_train_lmnn]
        for n_neighbors in candidate_n_neighbors:
            t = time.time()
            (filename_partdata, dic_day_test_labels_partdata, dic_day_estimated_label_partdata, dic_day_accuracy_partdata, \
                ...
                = run_with_lmnn('together', n_neighbors, n_train_lmnn, train_lmnn_vectors, train_lmnn_labels, \
                    train_vectors, all_train_labels_index, test_vectors, all_test_labels_index)
